      ...
      >
        <div className="bg-zinc-900/40 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-10 md:p-12 shadow-[0_40px_100px_rgba(0,0,0,0.8)] relative overflow-hidden group">
          ...
          <div className="mb-10 text-center">
            ...
            <h1 className="text-3xl font-light tracking-tight text-white mb-2">Recover Identity.</h1>
            <p className="text-[10px] font-mono text-white/30 uppercase tracking-[0.2em]">Reset your access passphrase</p>
          </div>

          {!submitted ? (
            <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} className="space-y-6">
              <p className="text-[11px] text-white/40 font-mono leading-relaxed uppercase tracking-wider mb-8">
                Enter the email associated with your node. We will broadcast a recovery handshake to your terminal.
              </p>
              ...
              <button className="w-full group relative py-5 bg-white text-black font-mono text-[11px] font-bold tracking-[0.3em] uppercase rounded-2xl transition-all duration-500 hover:scale-[1.02] active:scale-[0.98] shadow-2xl">
                <span className="relative z-10 flex items-center justify-center gap-3">
                  Send Handshake <Send className="w-4 h-4" />
                </span>
              </button>

              <Link to="/login" className="flex items-center justify-center gap-2 text-[10px] text-white/20 hover:text-white font-mono tracking-widest uppercase transition-all mt-8">
            ...
            <div className="text-center space-y-8 py-4">
              ...
              <h2 className="text-xl text-white font-medium">Handshake Broadcasted</h2>
              <p className="text-[11px] text-white/40 font-mono leading-relaxed uppercase tracking-wider">
                If an identity exists for that email, a recovery link has been beamed to your inbox.
              </p>
              <Link to="/login" className="inline-block px-12 py-4 bg-white/5 border border-white/10 rounded-xl text-[10px] text-white font-mono tracking-widest uppercase hover:bg-white/10 transition-all">
                Return to Login
              </Link>
            </div>
          )}
